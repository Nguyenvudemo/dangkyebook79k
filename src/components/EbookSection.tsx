    ...
    try {
      ...
      try {
        const response = await fetch('/api/register-lead', {
          ...
          body: JSON.stringify({
            ...formData,
            webhookUrl: localStorage.getItem('google_sheet_webhook_url') || ''
          })
        });
        ...
        if (contentType && contentType.includes("application/json")) {
          ...
          if (response.ok && parsedJson && parsedJson.success) {
            leadData = parsedJson.lead;
            if (parsedJson.sheetSyncStatus) {
              syncStatus = parsedJson.sheetSyncStatus;
              syncError = parsedJson.sheetError;
            }
          } else {
            throw new Error((parsedJson && parsedJson.error) || 'Đăng ký không thành công.');
          }
        } else {
          // Response is not JSON (e.g. 404 HTML from static host)
          console.warn("API returned non-JSON response. Falling back to client-side static storage.");
        }
      } catch (apiErr: any) {
        console.warn("API request failed or returned invalid response:", apiErr);
      }

      // If we didn't get valid registration data back from the backend API, handle as static fallback
      if (!leadData) {
        // --- CLIENT-SIDE FALLBACK FOR STATIC HOSTING (GitHub Pages, etc.) ---
        ...
        leadData = {
          ...
          niche: formData.niche?.trim() || "Nhận Quà Ebook",
          createdAt: new Date().toLocaleDateString('vi-VN') + ' ' + new Date().toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' })
        };

        // Persist locally in client-side localStorage
        const localLeads = JSON.parse(localStorage.getItem('registered_leads') || '[]');
        ...
        if (clientWebhook && clientWebhook.startsWith("http")) {
          try {
            ...
            await fetch(clientWebhook, {
              ...
              mode: "no-cors", // Crucial for direct Apps Script POST from client to prevent CORS blocks
              body: JSON.stringify({
                id: leadData.id,
                name: leadData.name,
            ...
            console.error("Client-side sync directly to Google Sheet Webhook failed:", sheetErr);
            syncStatus = "failed";
            syncError = sheetErr.message || "Không thể kết nối trực tiếp đến Webhook Google Sheet";
          }
        } else {
      ...
      // Update the sync status result state for UI feedback
      if (syncStatus === "success") {
        setSheetSyncResult({ status: 'success' });
      } else if (syncStatus === "failed" || syncStatus === "error") {
        setSheetSyncResult({ status: 'failed', error: syncError });
      } else {
        setSheetSyncResult({ status: 'local_only', error: null });
      }
            ...
            <div className="w-56 h-80 bg-gradient-to-tr from-indigo-800 via-indigo-600 to-emerald-500 rounded-r-2xl shadow-2xl transform transition-transform duration-500 hover:rotate-y-12 flex flex-col justify-between p-6 relative border-l-8 border-indigo-900 overflow-hidden">
              ...
              <div className="space-y-3">
                ...
                <h3 className="text-xl md:text-2xl font-bold font-space leading-tight tracking-tight text-white">
                  Ladipage <br/>
                  <span className="text-emerald-300">Chuẩn SEO</span>
                </h3>
                <div className="w-12 h-1 bg-emerald-400 rounded-full" />
                <p className="text-[11px] text-indigo-100 leading-snug">
                  Combo quà tặng Premium hoàn toàn Miễn Phí, bạn sẽ có 1 Ladipage thu hút để chia sẻ đến khách hàng trên ONLINE
                </p>
              </div>
        ...
        <div className="lg:col-span-7 flex flex-col justify-center">
          ...
          <div className="space-y-3 mb-6">
            ...
            <p className="text-lg md:text-2xl font-black font-space text-indigo-300 tracking-wide leading-tight drop-shadow-md">
              🎯 Đồng Bộ Hóa Google Sheet Tự Động Miễn Phí
            </p>
          </div>
          
          <p className="text-slate-300 text-sm md:text-base leading-relaxed mb-6 font-medium">
            Giúp bạn kết nối Form đăng ký trực tiếp về Google Sheet nhanh chóng, thu hút, đúng với nội dung mà khách hàng quan tâm.
          </p>
          ...
          <div className="space-y-2.5 mb-6">
            ...
            <div className="flex items-start gap-3">
              <CheckCircle className="w-5 h-5 text-emerald-400 shrink-0 mt-0.5" />
              <span className="text-slate-200 text-xs md:text-sm">
                <strong>Mở khóa quà tặng:</strong> Đăng ký giúp bạn nhận trọn bộ quà tặng Premium và file hướng dẫn kết nối Google Sheet.
              </span>
            </div>
          </div>
          ...
          {registeredLead ? (
              ...
              <div className="space-y-2">
                ...
                {sheetSyncResult ? (
                  <div className="mt-2 inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-950/80 border border-white/5 rounded-xl text-[11px] font-medium mx-auto">
                    ...
                    ) : sheetSyncResult.status === 'local_only' ? (
                      <>
                        <span className="w-2 h-2 rounded-full bg-amber-400 shrink-0" />
                        <span className="text-amber-400">Đã đăng ký thành công và lưu cục bộ!</span>
                      </>
                    ) : (
                      <>
            ...
            <form onSubmit={handleSubmit} className="bg-slate-900/80 border border-slate-700/80 rounded-2xl p-5 md:p-6 space-y-4 shadow-xl">
              <h4 className="text-sm md:text-base font-bold font-space flex items-center gap-1.5 text-emerald-400 border-b border-slate-800 pb-2">
                ...
                <span>Form Đăng Ký Nhận Ebook Quà Tặng Premium</span>
              </h4>
              ...
              >
                {isSubmitting ? (
                  ...
                  <>
                    <span>Nhận Quà Tặng & Đăng Ký Ngay</span>
                    <ArrowRight className="w-4 h-4" />
                  </>
                )}
              </button>
